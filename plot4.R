...
newDT1 <- data.frame(data$DateTime,data$Global_active_power)

newDT2 <-data.frame(data$DateTime,data$Voltage)

newDT3_1 <- data.frame(data$DateTime,data$Sub_metering_1)
newDT3_2 <- data.frame(data$DateTime,data$Sub_metering_2)
newDT3_3 <- data.frame(data$DateTime,data$Sub_metering_3)

newDT4 <-data.frame(data$DateTime,data$Global_reactive_power)
...
#setup each graph with xlabel, ylabel, and main title
